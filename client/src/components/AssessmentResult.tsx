...
export function AssessmentResult({ assessment }: AssessmentResultProps) {
  ...
  const factors =
    typeof assessment.factors === "string"
          ...
          {view === "patient" ? (
            ...
            >
              ...
              <div className="text-center space-y-4 max-w-2xl mx-auto">
                ...
                <div
                  className={`inline-flex flex-col items-center justify-center w-36 h-36 sm:w-48 sm:h-48rounded-full border-8 shadow-inner ${getRiskColor(assessment.riskCategory)}`}
                >
                ...
                <p className="text-muted-foreground text-lg">
                  Based on your provided information, your cardiovascular risk
                  over the next 10 years is considered{" "}
                  <strong>{assessment.riskCategory.toLowerCase()}</strong>.
                </p>
              </div>
              ...
              <div className="bg-secondary/50 rounded-xl p-6">
                <h3 className="font-bold text-lg mb-4 flex items-center gap-2">
                  <Info className="w-5 h-5 text-primary" /> What this means for
                  you
                </h3>
                <div className="grid gap-4 md:grid-cols-2">
                  {factors.map((factor: any, i: number) => (
                    <div
                      key={i}
                      className="flex gap-3 bg-card p-4 rounded-lg shadow-sm border border-border/50"
                    >
            ...
            >
              ...
              <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
                ...
                <div className="bg-card border border-border p-5 rounded-xl shadow-sm">
                  ...
                  <div
                    className={`inline-flex px-3 py-1 rounded-full text-sm font-bold mt-1 ${getRiskColor(assessment.riskCategory)}`}
                  >
                    {assessment.riskCategory}
                  </div>
                  {assessment.modelConfidence && (
                    <p className="text-[10px] text-muted-foreground mt-2 italic">
                      Model confidence:{" "}
                      {Number(assessment.modelConfidence).toFixed(2)}
                    </p>
                  )}
                </div>
                <div className="bg-card border border-border p-5 rounded-xl shadow-sm">
              ...
              <div className="bg-card border border-border rounded-xl p-4 sm:p-6 shadow-sm overflow-hidden">
                <h3 className="font-bold text-lg mb-6 flex items-center gap-2">
                  <Activity className="w-5 h-5 text-primary" /> Factor
                  Coefficient Impact
                </h3>
                          ...
                          if (active && payload && payload.length) {
                            ...
                            return (
                              <div className="bg-popover text-popover-foreground border border-border p-3 rounded-lg shadow-xl text-sm max-w-xs">
                                ...
                                <p
                                  className={`mt-2 font-semibold ${data.impact === "positive" ? "text-red-500" : "text-green-500"}`}
                                >
                                  Impact:{" "}
